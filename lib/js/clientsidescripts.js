/**
 * All scripts to be run on the client via executeAsyncScript or executeScript
 * should be put here.
 * 
 * NOTE: These scripts are transmitted over the wire as JavaScript text
 * constructed using their toString representation, and *cannot* reference
 * external variables.
 * 
 * Some implementations seem to have issues with // comments, so use star-style
 * inside scripts. (TODO: add issue number / example implementations that caused
 * the switch to avoid the // comments.)
 */
 ...
 * 
 * @param {function} fun The function to wrap up with its helpers @param
 * {...function} The helper functions. Each function must be named
 * 
...
function wrapWithHelpers(fun) {
	...
	return new FunClass(helpers.join(';') + String.fromCharCode(59)
 ...
 * 
 * @param {string} ngRepeat The ngRepeat to test @param {string} repeater The
 * repeater to test against @param {boolean} exact If the ngRepeat expression
 * needs to match the whole repeater (not counting any `track by ...` modifier)
 * or if it just needs to match a substring @return {boolean} If the ngRepeat
 * matched the repeater
 */
 ...
 * falsy, tries a variety of methods to find an injector @param {boolean=}
 * injectorPlease Prioritize finding an injector @return {$$testability?:
 * Testability, $injector?: Injector} Returns whatever ng1 app hooks it finds
 */
function getNg1Hooks(selector, injectorPlease) {
	...
	if (selector) {
		...
		return tryEl(document.body) || trySelector('[ng-app]')
				|| trySelector('[ng:app]') || trySelector('[ng-controller]')
...
/**
 * Wait until Angular has finished rendering and has no outstanding $http calls
 * before continuing. The specific Angular app is determined by the
 * rootSelector.
 * 
 * Asynchronous.
 * 
 * @param {string}
 *            rootSelector The selector housing an ng-app
 * @param {function(string)}
 *            callback callback. If a failure occurs, it will be passed as a
 *            parameter.
 */
functions.waitForAngular = function(rootSelector, callback) {
	try {
		if (window.angular
				&& !(window.angular.version && window.angular.version.major > 1)) {
			/* ng1 */
			...
			if (hooks.$$testability) {
				...
				throw new Error('root element (' + rootSelector
						+ ') has no injector.'
			...
			throw new Error(
					'window.angular is undefined.  This could be either '
							+ 'because this is a non-angular page or because your test involves '
							+ 'client-side navigation, which can interfere with Protractor\'s '
							+ 'bootstrapping.  See http://git.io/v4gXM for details');
		} else if (window.angular.version >= 2) {
			throw new Error(
					'You appear to be using angular, but window.'
 ...
 * 
 * @param {string}
 *            binding The binding, e.g. {{cat.name}}.
 * @param {boolean}
 *            exactMatch Whether the binding needs to be matched exactly
 * @param {Element}
 *            using The scope of the search.
 * @param {string}
 *            rootSelector The selector to use for the root app element.
 * 
...
functions.findBindings = function(binding, exactMatch, using, rootSelector) {
	...
	if (angular.getTestability) {
		return getNg1Hooks(rootSelector).$$testability.findBindings(using,
				binding, exactMatch);
	}
 ...
 * an array of only one element for plain old ng-repeat. Returns an array of all
 * the elements in one segment for ng-repeat-start.
 * 
 * @param {string}
 *            repeater The text of the repeater, e.g. 'cat in cats'.
 * @param {boolean}
 *            exact Whether the repeater needs to be matched exactly
 * @param {number}
 *            index The row index.
 * @param {Element}
 *            using The scope of the search.
...
function findRepeaterRows(repeater, exact, index, using) {
	...
	for (var p = 0; p < prefixes.length; ++p) {
		...
		for (var i = 0; i < repeatElems.length; ++i) {
			if (repeaterMatch(repeatElems[i].getAttribute(attr), repeater,
					exact)) {
...
functions.findAllRepeaterRows = wrapWithHelpers(findAllRepeaterRows,
		repeaterMatch);
 ...
 *            index The row index.
 * @param {string}
 *            binding The column binding, e.g. '{{cat.name}}'.
 * @param {Element}
 *            using The scope of the search.
 * @param {string}
 *            rootSelector The selector to use for the root app element.
 ...
 */
function findRepeaterElement(repeater, exact, index, binding, using,
		rootSelector) {
	var matches = [];
	using = using || document;
	...
	for (var p = 0; p < prefixes.length; ++p) {
		...
		for (var i = 0; i < repeatElems.length; ++i) {
			if (repeaterMatch(repeatElems[i].getAttribute(attr), repeater,
				...
				var row = [];
				while (elem.nodeType != 8
						|| (elem.nodeValue && !repeaterMatch(elem.nodeValue,
								repeater))) {
					if (elem.nodeType == 1) {
						row.push(elem);
					}
	...
	if (row) {
		if (angular.getTestability) {
			matches.push.apply(matches, getNg1Hooks(rootSelector).$$testability
					.findBindings(row, binding));
		} else {
...
functions.findRepeaterElement = wrapWithHelpers(findRepeaterElement,
		repeaterMatch, getNg1Hooks);
...
functions.findRepeaterColumn = wrapWithHelpers(findRepeaterColumn,
		repeaterMatch, getNg1Hooks);
...
functions.findByModel = function(model, using, rootSelector) {
	...
	if (angular.getTestability) {
		return getNg1Hooks(rootSelector).$$testability.findModels(using, model,
				true);
	}
	var prefixes = [ 'ng-', 'ng_', 'data-ng-', 'x-ng-', 'ng\\:' ];
	for (var p = 0; p < prefixes.length; ++p) {
 ...
 * 
 * @param {string}
 *            optionsDescriptor The descriptor for the option (i.e. fruit for
 *            fruit in fruits).
 * @param {Element}
 *            using The scope of the search.
 * 
...
functions.findByOptions = function(optionsDescriptor, using) {
	...
	var prefixes = [ 'ng-', 'ng_', 'data-ng-', 'x-ng-', 'ng\\:' ];
	for (var p = 0; p < prefixes.length; ++p) {
		var selector = '[' + prefixes[p] + 'options="' + optionsDescriptor
				+ '"] option';
		var elements = using.querySelectorAll(selector);
		if (elements.length) {
			return elements;
 ...
 * 
 * @param {string}
 *            searchText The exact text to match.
 * @param {Element}
 *            using The scope of the search.
 * 
 * @return {Array.<Element>} The matching elements.
 ...
 * 
 * @param {string}
 *            cssSelector The css selector to match.
 * @param {string}
 *            searchText The exact text to match.
 * @param {Element}
 *            using The scope of the search.
...
/**
 * Tests whether the angular global variable is present on a page. Retries in
 * case the page is just loading slowly.
 * 
 * Asynchronous.
 * 
 * @param {number}
 *            attempts Number of times to retry.
 * @param {boolean}
 *            ng12Hybrid Flag set if app is a hybrid of angular 1 and 2
 * @param {function({version:
 *            ?number, message: ?string})} asyncCallback callback
 * 
 */
		...
		try {
			...
			if (n < 1) {
				if (definitelyNg1 && window.angular) {
					...
					callback({
						message : 'angular 1 never loaded'
 ...
 * 
 * @param {Element}
 *            element The element in whose scope to evaluate.
 * @param {string}
 ...
 * 
 * @param {string}
 *            selector The selector housing an ng-app
 */
 ...
 *            selector The selector housing an ng-app
 * @param {string}
 *            url In page URL using the same syntax as $location.url(),
 ...
 * exec[Async]Script. In addition, also include a script that will install all
 * the functions on window (for debugging.)
 * 
 * We also wrap any exceptions thrown by a clientSideScripts function that is
 * not an instance of the Error type into an Error type. If we don't do so, then
 * the resulting stack trace is completely unhelpful and the exception message
 * is just "unknown error." These types of exceptions are the common case for
 * dart2js code. This wrapping gives us the Dart stack trace and exception
 * message.
 */
...
exports.installInBrowser = (util.format('window.clientSideScripts = {%s};',
		scriptsList.join(', ')));

/**
 * Automatically installed by Protractor when a page is loaded, this default
 * mock module decorates $timeout to keep track of any outstanding timeouts.
 * 
 * @param {boolean}
 *            trackOutstandingTimeouts
 */
